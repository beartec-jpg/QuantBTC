/*
 * Wrapper for implementing the PQClean API — Falcon-padded-1024.
 *
 * This file is the Falcon-1024 counterpart of the Falcon-512 pqclean.c.
 * Key differences from Falcon-512:
 *   logn = 10  (degree n = 1024, vs 9/512 for Falcon-512)
 *   CRYPTO_SECRETKEYBYTES = 2305
 *   CRYPTO_PUBLICKEYBYTES = 1793
 *   CRYPTO_BYTES = 1280
 *   sk header byte: 0x50 + 10 = 0x5A
 *   pk header byte: 0x00 + 10 = 0x0A
 *   sig header byte: 0x30 + 10 = 0x3A
...
PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_keypair(
    ...
    if (v == 0) { return -1; }
    u += v;
    if (u != PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_SECRETKEYBYTES) { return -1; }

    pk[0] = 0x00 + 10;
    v = PQCLEAN_FALCONPADDED1024_CLEAN_modq_encode(
            pk + 1, PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_PUBLICKEYBYTES - 1,
            h, 10);
    if (v != PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_PUBLICKEYBYTES - 1) { return -1; }

    return 0;
}

/* Deterministic key pair from 48-byte seed — used by QuantBTC HD derivation. */
int
PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_seed_keypair(
...
do_sign(uint8_t *nonce, uint8_t *sigbuf, size_t sigbuflen,
    ...
    u += v;
    if (u != PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_SECRETKEYBYTES) { return -1; }
    if (!PQCLEAN_FALCONPADDED1024_CLEAN_complete_private(G, f, g, F, 10, tmp.b)) { return -1; }
...
do_verify(
    ...
    if (!PQCLEAN_FALCONPADDED1024_CLEAN_verify_raw(hm, sig, h, 10, tmp.b)) { return -1; }
    return 0;
}
...
PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_signature(
    ...
    if (do_sign(sig + 1, sig + 1 + NONCELEN, vlen, m, mlen, sk) < 0) { return -1; }
    sig[0] = 0x30 + 10;
...
PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_verify(
    ...
    const uint8_t *m, size_t mlen, const uint8_t *pk) {
    if (siglen < 1 + NONCELEN) { return -1; }
    if (sig[0] != 0x30 + 10) { return -1; }
...
PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign(
    ...
    if (do_sign(sm + 1, sigbuf, sigbuflen, m, mlen, sk) < 0) { return -1; }
    sm[0] = 0x30 + 10;
    sigbuflen++;
...
PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_open(
    ...
    if (smlen < PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES) { return -1; }
    sigbuflen = PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES - NONCELEN - 1;
    pmlen = smlen - PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES;
    if (sm[0] != 0x30 + 10) { return -1; }
    sigbuf = sm + 1 + NONCELEN;
