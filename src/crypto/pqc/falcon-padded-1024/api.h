...
/*
 * Generate a key pair. Public key goes into pk[], private key into sk[].
...
int PQCLEAN_FALCONPADDED1024_CLEAN_crypto_sign_seed_keypair(
    uint8_t *pk, uint8_t *sk,
    const uint8_t *seed);

/*
 * Compute a signature on message m[] of length mlen.
 * Signature length (in bytes) is PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES.
 */
...
/*
 * Verify a signature. Returns 0 on success, -1 on error.
 */
...
/*
 * Sign a message; output is the signed message (sig || msg).
 * Written length may range up to PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES.
 */
...
/*
 * Verify signed message. Returns 0 on success, -1 on failure.
 * Output message length may range up to PQCLEAN_FALCONPADDED1024_CLEAN_CRYPTO_BYTES.
 */
