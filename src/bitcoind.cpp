...
static bool ParseArgs(ArgsManager& args, int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see quantumbtcd -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        std::string strUsage = "QuantumBTC version " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\nUsage:  quantumbtcd [options]                     Start QuantumBTC\n"
                "\n";
...
#if HAVE_DECL_FORK
            tfm::format(std::cout, "QuantumBTC starting\n");
